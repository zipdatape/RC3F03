...
## 🔍 DIAGNÓSTICO: ¿De qué política GPO viene el reinicio?

### ⚠️ POLÍTICA GPO IDENTIFICADA

**Nombre de la Política:** `__WSUS`  
**GUID:** `673de858-cf21-426d-80f1-8b74c3801949`  
**Última modificación:** 10 de octubre de 2025, 13:05:10  
**Estado:** Habilitada (AllSettingsEnabled)  
**Dominio:** ADPER.HITSS.MX

### Configuración Actual Detectada

Basado en el análisis del registro y políticas de grupo, tu equipo tiene las siguientes políticas aplicadas:

#### Políticas de Windows Update (GPO)
**Ruta del registro:** `HKLM:\SOFTWARE\Policies\Microsoft\Windows\WindowsUpdate\AU`  
**Política aplicada:** `__WSUS`

| Parámetro | Valor Actual | Significado | Política |
|-----------|--------------|-------------|----------|
| **AUOptions** | `7` | **Descargar e instalar automáticamente, luego reiniciar automáticamente** | **__WSUS** |
| **ScheduledInstallDay** | `7` | Todos los días | **__WSUS** |
| **ScheduledInstallTime** | `3` | **3:00 AM** (hora de instalación) | **__WSUS** |
...
| **AutoInstallMinorUpdates** | `1` | Instalar actualizaciones menores automáticamente | **__WSUS** |
| **NoAutoUpdate** | `0` | Actualizaciones automáticas habilitadas | **__WSUS** |

### Política GPO Aplicada

La política que está causando los reinicios automáticos es:
...
- **Horario programado:** Todos los días a las 3:00 AM
- **Servidor WSUS:** `wsus.adper.hitss.mx:8530`

**Para modificar esta política:**
1. Abrir **Administración de directivas de grupo** (gpmc.msc)
2. Buscar la política: **`__WSUS`**
...
## ⚙️ CÓMO CONTROLAR EL HORARIO DE REINICIO

### Opción 1: Modificar la Política de Grupo (Recomendado para dominio)

Si tienes acceso al servidor de políticas de grupo:

#### Paso 1: Abrir Editor de Políticas de Grupo
1. Abre **Administración de directivas de grupo** (gpmc.msc)
2. **Buscar la política:** `__WSUS` (GUID: 673de858-cf21-426d-80f1-8b74c3801949)
...
3. En **"Configure automatic updating"**, selecciona una de estas opciones:
   ...
   | **3** | Descargar automáticamente y notificar antes de instalar |
   | **4** | **Descargar automáticamente e instalar en horario programado** (recomendado) |
   | **5** | Permitir al administrador local elegir |

4. Si eliges la opción **4**, configura:
   - **Scheduled install day:** 
     - `0` = Todos los días
     - `1-7` = Domingo a Sábado
     - `8` = Solo los días laborables
...
### Opción 2: Modificar Registro Localmente (Solo para este equipo)

Si no tienes acceso a GPO o quieres una solución temporal:
...
param(
    ...
    [Parameter(Mandatory=$false)]
    [int]$DiaInstalacion = 0,     # 0 = Todos los días, 1-7 = Domingo-Sábado
...
if ($DiaInstalacion -lt 0 -or $DiaInstalacion -gt 8) {
    Write-Error "El día debe estar entre 0 (todos) y 8 (laborables)"
    exit 1
}

try {
    # Cambiar horario de instalación
    Write-Host "Configurando horario de instalación: $HoraInstalacion:00" -ForegroundColor Yellow
    Set-ItemProperty -Path $auPath -Name "ScheduledInstallTime" -Value $HoraInstalacion -Type DWord -ErrorAction Stop
    
    # Cambiar día de instalación
    ...
    Write-Host "Configurando día de instalación: $diaTexto" -ForegroundColor Yellow
    ...
    Write-Host "Cambiando modo de actualización a 'Instalar en horario programado'" -ForegroundColor Yellow
    Set-ItemProperty -Path $auPath -Name "AUOptions" -Value 4 -Type DWord -ErrorAction Stop
    
    Write-Host "`nConfiguración aplicada exitosamente!" -ForegroundColor Green
    Write-Host "Horario de instalación: $HoraInstalacion:00" -ForegroundColor Green
    Write-Host "Día: $diaTexto" -ForegroundColor Green
    ...
    Write-Error "Error al aplicar configuración: $($_.Exception.Message)"
    exit 1
}
```
...
### AUOptions (Modo de Actualización)
...
| 4 | **Descargar automáticamente e instalar en horario programado** ⭐ Recomendado |
| 5 | Permitir al administrador local elegir |
| 7 | Descargar e instalar automáticamente, luego reiniciar automáticamente (actual) |

### ScheduledInstallDay (Día de Instalación)
...
| 6 | Viernes |
| 7 | Sábado |
| 8 | Solo días laborables |

### ScheduledInstallTime (Hora de Instalación)
...
if ($au) {
    ...
    Write-Host "No hay políticas de GPO aplicadas" -ForegroundColor Yellow
}
```
...
1. **Si modificas el registro localmente:**
   - Los cambios pueden ser sobrescritos por GPO en el próximo refresh
   - Ejecuta `gpupdate /force` después de modificar GPO
   - Reinicia el servicio: `Restart-Service wuauserv -Force`

2. **Para cambios permanentes:**
   - Modifica la política de grupo en el servidor
   - Los cambios se aplicarán a todos los equipos del dominio/OU

3. **Recomendaciones de horario:**
   - Horas de menor uso: 10:00 PM - 2:00 AM
   - Días: Sábados o domingos para actualizaciones grandes
...
**Problema actual:**
- Reinicios automáticos a las 3:00 AM todos los días
- Causado por `AUOptions = 7` (reinicio automático inmediato)

**Solución recomendada:**
1. Cambiar `AUOptions` de `7` a `4` (instalar en horario programado)
2. Cambiar `ScheduledInstallTime` a una hora más conveniente (ej: 22 = 10:00 PM)
3. Configurar horarios activos (8:00 AM - 6:00 PM)
4. Aumentar `RebootRelaunchTimeout` a 30-60 minutos

**Próximos pasos:**
1. Identificar la política GPO en el servidor
2. Modificar la política o aplicar cambios locales
3. Verificar que los cambios se aplicaron correctamente
4. Monitorear los próximos reinicios
